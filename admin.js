...
function getData(){ const saved = localStorage.getItem("ramsesSiteData"); return saved ? { ...defaultData, ...JSON.parse(saved)} : defaultData; }
function collectForm(){ const data={}; Object.entries(fields).forEach(([id,key])=>{ const el=document.getElementById(id); if(el) data[key]=el.value;}); return data; }
function renderPreview(){ const data=collectForm(); document.getElementById("previewCategory").textContent=data.heroCategory||"ملف خاص"; document.getElementById("previewTitle").textContent=data.heroTitle||"عنوان المقال الرئيسي"; document.getElementById("previewDesc").textContent=data.heroDesc||"وصف قصير"; }
function populateForm(){ const data=getData(); Object.entries(fields).forEach(([id,key])=>{ const el=document.getElementById(id); if(el) el.value=data[key]; }); renderPreview(); }
populateForm();
document.querySelectorAll("input, textarea").forEach(el=>el.addEventListener("input",renderPreview));
document.getElementById("saveData").addEventListener("click",()=>{ localStorage.setItem("ramsesSiteData", JSON.stringify(collectForm())); alert("تم حفظ الألوان والهوية العامة داخل المتصفح."); });
document.getElementById("resetData").addEventListener("click",()=>{ localStorage.removeItem("ramsesSiteData"); populateForm(); alert("تمت استعادة الإعدادات الافتراضية."); });
