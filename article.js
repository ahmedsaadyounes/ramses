...
function renderArticle() {
  ...
  document.getElementById("articleSidebarText").textContent = uiText[currentLang].articleSidebarText;
  ...
  document.getElementById("articleBody").innerHTML = t.content.map((p) => `<p>${p}</p>`).join("") + `
    <blockquote>${currentLang === "ar" ? "رمسيس يقدّم المقالات بطريقة تحافظ على فخامة الشكل وسهولة القراءة." : "Ramses presents articles with a premium feel and an easy, polished reading experience."}</blockquote>`;
